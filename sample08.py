...
'''
sample01を元に、３入力にしてみる。（とりあえずANDで）
'''
...
class SmallClassificationModel(chainer.FunctionSet):
    def __init__(self):
        #bmodel definition
        h1_num = 3  # １層あたりのユニット数
        super(SmallClassificationModel, self).__init__(
            fc1 = chainer.functions.Linear(3, h1_num)
            )
    ...
    def train(self, x_data, y_data):
        x = chainer.Variable(x_data.reshape(1,3).astype(numpy.float32), volatile=False)
...
data_and = [
    ...
    [numpy.array([0,1,0]), numpy.array([0])],
    [numpy.array([1,0,0]), numpy.array([0])],
    [numpy.array([1,1,0]), numpy.array([0])],
    [numpy.array([0,0,1]), numpy.array([0])],
    [numpy.array([0,1,1]), numpy.array([0])],
    [numpy.array([1,0,1]), numpy.array([0])],
    [numpy.array([1,1,1]), numpy.array([1])],
]*1000
